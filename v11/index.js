var Godot = (()=>{
    ...
    return (function(Godot={}) {
        ...
        function ___syscall_chdir(path) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(7, 1, path);
            try {
        ...
        function ___syscall_chmod(path, mode) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(8, 1, path, mode);
            try {
                path = SYSCALLS.getStr(path);
                FS.chmod(path, mode);
                return 0;
        ...
        function ___syscall_listen(fd, backlog) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(18, 1, fd, backlog);
            try {
                var sock = getSocketFromFD(fd);
        ...
        function ___syscall_lstat64(path, buf) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(19, 1, path, buf);
            try {
                path = SYSCALLS.getStr(path);
        ...
        function ___syscall_rmdir(path) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(27, 1, path);
            try {
                path = SYSCALLS.getStr(path);
                FS.rmdir(path);
                return 0;
        ...
        function ___syscall_stat64(path, buf) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(30, 1, path, buf);
            try {
                path = SYSCALLS.getStr(path);
        ...
        function _emscripten_force_exit(status) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(35, 1, status);
            noExitRuntime = false;
            runtimeKeepaliveCounter = 0;
            _exit(status);
        }
        ...
        function _fd_close(fd) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(42, 1, fd);
            try {
                var stream = SYSCALLS.getStreamFromFD(fd);
                FS.close(stream);
                return 0;
        ...
        function _fd_fdstat_get(fd, pbuf) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(43, 1, fd, pbuf);
            try {
                var rightsBase = 0;
        ...
        function _godot_audio_worklet_create(channels) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(54, 1, channels);
            try {
        ...
        function _godot_js_display_clipboard_get(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(61, 1, callback);
        ...
        function _godot_js_display_clipboard_set(p_text) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(62, 1, p_text);
        ...
        function _godot_js_display_cursor_lock_set(p_lock) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(65, 1, p_lock);
            if (p_lock) {
        ...
        function _godot_js_display_fullscreen_cb(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(70, 1, callback);
            const canvas = GodotConfig.canvas;
            const func = GodotRuntime.get_func(callback);
        ...
        function _godot_js_display_has_webgl(p_version) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(73, 1, p_version);
        ...
        function _godot_js_display_window_blur_cb(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(87, 1, callback);
            const func = GodotRuntime.get_func(callback);
        ...
        function _godot_js_display_window_icon_set(p_ptr, p_len) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(88, 1, p_ptr, p_len);
        ...
        function _godot_js_display_window_title_set(p_data) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(90, 1, p_data);
        ...
        function _godot_js_fetch_create(p_method, p_url, p_headers, p_headers_size, p_body, p_body_size) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(91, 1, p_method, p_url, p_headers, p_headers_size, p_body, p_body_size);
        ...
        function _godot_js_fetch_free(id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(92, 1, id);
            GodotFetch.free(id);
        }
        function _godot_js_fetch_http_status_get(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(93, 1, p_id);
        ...
        function _godot_js_fetch_is_chunked(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(94, 1, p_id);
            const obj = IDHandler.get(p_id);
            if (!obj || !obj.response) {
                return -1;
            }
        ...
        function _godot_js_fetch_state_get(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(97, 1, p_id);
            const obj = IDHandler.get(p_id);
            if (!obj) {
                return -1;
            }
        ...
        function _godot_js_input_drop_files_cb(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(98, 1, callback);
            const func = GodotRuntime.get_func(callback);
        ...
        function _godot_js_input_gamepad_sample_get(p_index, r_btns, r_btns_num, r_axes, r_axes_num, r_standard) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(102, 1, p_index, r_btns, r_btns_num, r_axes, r_axes_num, r_standard);
        ...
        function _godot_js_input_mouse_button_cb(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(104, 1, callback);
            const func = GodotRuntime.get_func(callback);
            const canvas = GodotConfig.canvas;
        ...
        function _godot_js_input_mouse_move_cb(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(105, 1, callback);
            const func = GodotRuntime.get_func(callback);
            const canvas = GodotConfig.canvas;
        ...
        function _godot_js_input_mouse_wheel_cb(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(106, 1, callback);
            const func = GodotRuntime.get_func(callback);
        ...
        function _godot_js_input_paste_cb(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(107, 1, callback);
            const func = GodotRuntime.get_func(callback);
        ...
        function _godot_js_os_execute(p_json) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(111, 1, p_json);
        ...
        function _godot_js_os_finish_async(p_callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(112, 1, p_callback);
            const func = GodotRuntime.get_func(p_callback);
            GodotOS.finish_async(func);
        }
        function _godot_js_os_fs_is_persistent() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(113, 1);
            return GodotFS.is_persistent();
        }
        function _godot_js_os_fs_sync(callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(114, 1, callback);
            const func = GodotRuntime.get_func(callback);
        ...
        function _godot_js_os_has_feature(p_ftr) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(115, 1, p_ftr);
        ...
        function _godot_js_pwa_cb(p_update_cb) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(119, 1, p_update_cb);
        ...
        function _godot_js_pwa_update() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(120, 1);
        ...
        function _godot_js_rtc_datachannel_close(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(121, 1, p_id);
            const ref = IDHandler.get(p_id);
            if (!ref) {
                return;
            }
        ...
        function _godot_js_rtc_datachannel_connect(p_id, p_ref, p_on_open, p_on_message, p_on_error, p_on_close) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(122, 1, p_id, p_ref, p_on_open, p_on_message, p_on_error, p_on_close);
        ...
        function _godot_js_rtc_datachannel_max_packet_lifetime_get(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(129, 1, p_id);
            const ref = IDHandler.get(p_id);
            if (!ref) {
                return 65535;
            }
        ...
        function _godot_js_rtc_datachannel_max_retransmits_get(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(130, 1, p_id);
        ...
        function _godot_js_rtc_datachannel_ready_state_get(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(131, 1, p_id);
            const ref = IDHandler.get(p_id);
            if (!ref) {
                return 3;
            }
        ...
        function _godot_js_rtc_pc_close(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(133, 1, p_id);
            const ref = IDHandler.get(p_id);
            if (!ref) {
                return;
            }
        ...
        function _godot_js_rtc_pc_create(p_config, p_ref, p_on_connection_state_change, p_on_ice_gathering_state_change, p_on_signaling_state_change, p_on_ice_candidate, p_on_datachannel) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(134, 1, p_config, p_ref, p_on_connection_state_change, p_on_ice_gathering_state_change, p_on_signaling_state_change, p_on_ice_candidate, p_on_datachannel);
        ...
        function _godot_js_rtc_pc_destroy(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(136, 1, p_id);
        ...
        function _godot_js_tts_get_voices(p_callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(141, 1, p_callback);
            const func = GodotRuntime.get_func(p_callback);
            try {
                const arr = [];
        ...
        function _godot_js_tts_is_paused() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(142, 1);
            return window.speechSynthesis.paused;
        }
        function _godot_js_tts_is_speaking() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(143, 1);
            return window.speechSynthesis.speaking;
        }
        function _godot_js_tts_pause() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(144, 1);
            window.speechSynthesis.pause();
        }
        function _godot_js_tts_resume() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(145, 1);
            window.speechSynthesis.resume();
        }
        function _godot_js_tts_speak(p_text, p_voice, p_volume, p_pitch, p_rate, p_utterance_id, p_callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(146, 1, p_text, p_voice, p_volume, p_pitch, p_rate, p_utterance_id, p_callback);
            const func = GodotRuntime.get_func(p_callback);
        ...
        function _godot_js_tts_stop() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(147, 1);
        ...
        function _godot_js_websocket_buffered_amount(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(148, 1, p_id);
        ...
        function _godot_js_websocket_create(p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(150, 1, p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close);
        ...
        function _godot_js_websocket_destroy(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(151, 1, p_id);
        ...
        function _godot_js_wrapper_create_object(p_object, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(154, 1, p_object, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback);
        ...
        function _godot_js_wrapper_interface_get(p_name) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(155, 1, p_name);
            const name = GodotRuntime.parseString(p_name);
            if (typeof window[name] !== "undefined") {
        ...
        function _godot_js_wrapper_object_call(p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(156, 1, p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback);
        ...
        function _godot_js_wrapper_object_unref(p_id) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(162, 1, p_id);
        ...
        function _godot_webgl2_glFramebufferTextureMultiviewOVR(target, attachment, texture, level, base_view_index, num_views) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(163, 1, target, attachment, texture, level, base_view_index, num_views);
        ...
        function _godot_webxr_get_bounds_geometry(r_points) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(165, 1, r_points);
        ...
        function _godot_webxr_get_color_texture() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(166, 1);
        ...
        function _godot_webxr_get_depth_texture() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(167, 1);
            const subimage = GodotWebXR.getSubImage();
            if (subimage === null) {
                return 0;
            }
        ...
        function _godot_webxr_get_frame_rate() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(168, 1);
        ...
        function _godot_webxr_get_render_target_size(r_size) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(170, 1, r_size);
            const subimage = GodotWebXR.getSubImage();
            if (subimage === null) {
                return false;
            }
        ...
        function _godot_webxr_get_velocity_texture() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(173, 1);
            const subimage = GodotWebXR.getSubImage();
            if (subimage === null) {
                return 0;
            }
        ...
        function _godot_webxr_get_view_count() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(174, 1);
            if (!GodotWebXR.session || !GodotWebXR.pose) {
                return 1;
            }
        ...
        function _godot_webxr_get_visibility_state() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(175, 1);
        ...
        function _godot_webxr_initialize(p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(176, 1, p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event);
        ...
        function _godot_webxr_is_supported() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(178, 1);
            return !!navigator.xr;
        }
        function _godot_webxr_uninitialize() {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(179, 1);
        ...
        function _godot_webxr_update_input_source(p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes) {
            if (ENVIRONMENT_IS_PTHREAD)
                return proxyToMainThread(180, 1, p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes);
